...
async function _valhallaMultiIsochrone(contour) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      if (!resp.ok) { clearTimeout(tid); continue; }
      const data = await resp.json();
      clearTimeout(tid);
      if (data.features?.length) {
        _valhallaWorkingUrl = url;
        // Return the largest polygon (main continent, not tiny islands)
        return _largestFeature(data.features);
      }
    } catch (err) {
      clearTimeout(tid);
      if (err.name !== 'AbortError') continue;
...
/**
 * From an array of GeoJSON features, return the one with the largest bbox area.
 * Discards tiny island fragments from Valhalla response.
 */
function _largestFeature(features) {
  ...
  return features.reduce((best, f) => {
    try {
      ...
      const bestBb = turf.bbox(best);
      const bestArea = (bestBb[2]-bestBb[0]) * (bestBb[3]-bestBb[1]);
