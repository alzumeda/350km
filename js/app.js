...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Set icon
  const _radiusIcon = document.getElementById('btn-radius')?.querySelector('.icon');
  if (_radiusIcon) _radiusIcon.textContent = '⚙';
  ...
  // Hide splash screen — map is ready
  ...
  function _showOfflineBanner(offline) {
    ...
    if (!banner) {
      ...
      banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9998;' +
        'background:#e94560;color:white;text-align:center;font-size:13px;' +
        'font-family:system-ui,sans-serif;padding:6px;display:none;';
      banner.textContent = '📡 Offline — Karte aus Cache, API-Funktionen nicht verfügbar';
      document.body.appendChild(banner);
    }
    banner.style.display = offline ? 'block' : 'none';
  }
  window.addEventListener('online',  () => { _showOfflineBanner(false); showToast('✓ Wieder online'); });
  ...
  window.addEventListener('beforeinstallprompt', e => {
    e.preventDefault();
    _installPrompt = e;
    // Show install button in settings if not already installed
    const btn = document.getElementById('sett-install-btn');
    if (btn) btn.style.display = 'flex';
  });

  } catch(err) {
    console.error('[app] init error:', err.message, err.stack?.split('\n')?.[1]);
