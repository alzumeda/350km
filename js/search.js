...
let _lastNomReqTime = 0;
const NOM_MIN_INTERVAL = 1050; // Nominatim ToS: max 1 req/second

async function _doSearch(query) {
  try {
  // Rate-limit: ensure min 1050ms between Nominatim requests
  const now = Date.now();
  const wait = NOM_MIN_INTERVAL - (now - _lastNomReqTime);
  if (wait > 0) await new Promise(r => setTimeout(r, wait));
  _lastNomReqTime = Date.now();
