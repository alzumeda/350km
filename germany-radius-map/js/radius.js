...
function _repairGeom(geom) {
  if (!geom) return geom;
  try { return turf.buffer(geom, 0); } catch { return geom; }
}

function _safeUnion(a, b) {
  if (!a) return b;
  if (!b) return a;
  try { return turf.union(_repairGeom(a), _repairGeom(b)); }
  catch (e) {
...
function _ensureGermanyIncluded(geom) {
  ...
  return _safeUnion(geom, GERMANY_BORDER);
}
...
async function _valhallaIsochrone(location, contour) {
  const ctrl = new AbortController();
  const tid  = setTimeout(() => ctrl.abort(), 60000); // 60s — large isochroness need time
  try {
...
async function _drawValhallaIsochrone(mode) {
  ...
  try {
    if (mode === 'time') {
      ...
      const timeResults = await Promise.all(
        DE_BORDER_KEY_POINTS.map(async pt => {
          const feat = await _valhallaIsochrone(pt, { time: timeMins });
          timeDone++;
          progress.style.transform = `scaleX(${0.1 + 0.75 * (timeDone / DE_BORDER_KEY_POINTS.length)})`;
          return feat;
        })
      );
    ...
    // Use full radiusKm — Valhalla has no range limit (unlike ORS free tier)
    const distKm = State.radiusKm;
    showToast(`Valhalla: ${DE_BORDER_KEY_POINTS.length} Punkte × ${distKm}km — kann 1-2 Min dauern…`);
    progress.style.display = 'block';
    progress.style.transform = 'scaleX(0.1)';

    // Fix 1: overall timeout — scaled to distance (350km needs more time than 100km)
    const overallCtrl = new AbortController();
    const overallTimeout = Math.min(120000, 30000 + distKm * 150); // 30s base + 150ms/km
    const overallTid  = setTimeout(() => overallCtrl.abort(), overallTimeout);
