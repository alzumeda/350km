  ...
  State.routeControl.on('routesfound', ev => {
    ...
    if (latLngs.length) {
      fetchAndShowElevation(latLngs).then(() => {
        // After elevation loads: recalculate corrected travel time
    ...
    if (routeCrossing && State.userPos) {
      ...
      _osrmDuration && _osrmDistToPoint(uLat, uLng, routeCrossing.lat, routeCrossing.lng)
        .then(res => {
          if (!res) return;
          const within = res.driveKm <= State.radiusKm;
          document.getElementById('route-border-dist').textContent =
            `${res.driveKm.toFixed(1)} km ${within ? '✓' : '✗'}`;
        });
    }
  });
